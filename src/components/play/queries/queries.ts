...
// requests
import { 
  getTriviaQuestions, getTriviaTags,
  getTriviaCategories
} from "../../../service/axios";
// type
...
export const useFreeTriviaQuestions = ({ limit, categories, difficulty, tags }: Concrete<IQueryObj>) => {
  ...
  return useQuery(
    ['trivia-free', limit, categories, difficulty, tags], 
    getTriviaQuestions, 
...
export const useTimedTriviaQuestions = ({ limit, categories, difficulty, tags }: Concrete<IQueryObj>) => {
  ...
  return useQuery(
    ['trivia-timed', limit, categories, difficulty, tags], 
    getTriviaQuestions, 
    {refetchOnWindowFocus: false }
  );
};
...
export const useGetTriviaTags = () => {
  return useQuery(
    ['trivia-tags'], 
    getTriviaTags, 
    {refetchOnWindowFocus: false, staleTime: 86400000, placeholderData: [
      "arts_and_literature", "biology", "business", "chemistry",
    ]}
  );
}