...
// query
import { 
  useGetTriviaTags, useGetTriviaCategories 
...
export default function TimedPlaySetup () {
  ...
  // store
  const { dispatchToStore, timedPlaySettings } = useStore();
  // tags
  ...
  return(
    ...
    >
      ...
      <StyledOptionDiv>
        <Typography variant={"caption"} gutterBottom={true} sx={{marginBottom: 2}}> Time </Typography>
        <Stack direction={"row"} flexWrap={"wrap"} justifyContent={"center"}>
          <TextField id="outlined-minutes" label="Minutes" type="number" value={timedPlaySettings.minutes}
            InputLabelProps={{shrink: true}}
            onChange={(e) => handleChange("min", e.target.value)}
            sx={{m:1}}
          />
          <TextField id="outlined-seconds" label="Seconds" type="number" value={timedPlaySettings.seconds}
            InputLabelProps={{shrink: true,}}
